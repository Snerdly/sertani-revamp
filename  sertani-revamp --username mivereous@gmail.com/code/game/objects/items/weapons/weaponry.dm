...
/obj/item/weapon/katana/cyber
	name = "sleek blade"
	desc = "A modernized version of the Japanese katana, featuring space age sharpening techniques, stronger materials, and an electrified Mune for incapacitation."
...
/obj/item/weapon/katana/cyber/update_icon()
	if(status)
		icon_state = "stun_blade_active"
	else
		icon_state = "stun_blade"

/obj/item/weapon/katana/cyber/attack_self(mob/user as mob)
	if(status && (CLUMSY in user.mutations) && prob(50))
		user << "\red You grab the [src] by the mune."
		user.Weaken(30)
		charges--
		if(charges < 1)
			status = 0
	...
	if(charges > 0)
		status = !status
		user << "<span class='notice'>\The [src] is now [status ? "live" : "off"].</span>"
...
/obj/item/weapon/katana/cyber/attack(mob/M as mob, mob/user as mob)
	if(status && (CLUMSY in user.mutations) && prob(50))
		user << "<span class='danger'>You accidentally brush yourself with the back of the [src]!</span>"
		user.Weaken(30)
		charges--
		if(charges < 1)
			status = 0
	...
	if(status)
		...
		if(isrobot(src.loc))
			...
			charges--
		H.visible_message("<span class='danger'>[M] has been struck with the [src] by [user]!</span>")
	...
	if (prob(25))//Because the area of impact is smaller than that of a baton.
		if(istype(hit_atom, /mob/living))
			...
			if(status)
				...
				H.visible_message("<span class='danger'>[src], skillfully thrown by [foundmob.name], strikes [H] and stuns them!</span>")
...
/obj/item/weapon/katana/cyber/emp_act(severity)
	switch(severity)
		if(1)
			charges = 0
		if(2)
