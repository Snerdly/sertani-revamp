	...
	proc/update_preview_icon()		//seriously. This is horrendous.
		...
		if(job_civilian_low & ASSISTANT)//This gives the preview icon clothes depending on which job(if any) is set to 'high'
			...
			switch(job_engsec_high)
				...
				if(PROTECTOR)
					clothes_s = new /icon('icons/mob/uniform.dmi', "protector_s")
					clothes_s.Blend(new /icon( 'icons/mob/feet.dmi', "swat"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon( 'icons/mob/hands.dmi', "bgloves"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon( 'icons/mob/suit.dmi', "protector_coat"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/items_lefthand.dmi', "stun_blade"), ICON_UNDERLAY)
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "securitypack"), ICON_OVERLAY)
						if(3)
