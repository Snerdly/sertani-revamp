...
/datum/trade_destination/centcomm
	name = "CentComm"
	description = "Saris Industries's administrative centre for Tau Ceti."
	distance = 1.2
	willing_to_buy = list()
...
/datum/trade_destination/anansi/get_custom_eventstring(var/event_type)
	if(event_type == RESEARCH_BREAKTHROUGH)
		...
		[pick("mind-machine interfacing","neuroscience","nano-augmentation","genetics")]. Saris Industries is expected to announce a co-exploitation deal within the fortnight."
	return null
...
/datum/trade_destination/beltway
	...
	description = "A co-operative effort between Beltway and Saris Industries to exploit the rich outer asteroid belt of the Tau Ceti system."
	distance = 7.5
	willing_to_buy = list()
	willing_to_sell = list()
