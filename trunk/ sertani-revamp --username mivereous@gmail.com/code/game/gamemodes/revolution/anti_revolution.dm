...
/datum/game_mode/anti_revolution/proc/add_head_objectives(datum/mind/head)
	...
	for(var/datum/mind/target in demote_targets)
		...
		obj.owner = head
		obj.target = target
		obj.explanation_text = "[target.current.real_name], the [target.assigned_role]  has been classified as harmful to Saris Industries's goals. Demote them to assistant."
		head.objectives += obj
